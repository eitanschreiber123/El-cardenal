...
const transporter=nodemailer.createTransport({host:"smtp.gmail.com",port:465,secure:true,auth:{user:"eitanschreiber97@gmail.com",pass:emailPass},tls:{rejectUnauthorized:false}})
export default async(req,res)=>{const{senderMail,name,origin,affair,message}=req.body
    if(name ===""||origin===""||affair===""||message===""){res.status(403).send("")
        return
